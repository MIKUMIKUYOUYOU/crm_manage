...
var initbootstrapValidator = function(fm) {
	fm.bootstrapValidator(
			...
			function(e) {
				// 阻止表单提交
				e.preventDefault();
				var param = $positionFrom.serialize();
...
let addAndUpdate = function(hidePageNode, showPageNode, method, btnNode) {
	...
	if (method == "update") {
		...
		// 隐藏input域用于修改页面的任务
		$("#method").val("UPDATE");
		var trChildNode = btnNode.parentNode.parentNode.parentNode.childNodes;
		$("#POSITION_ID").val(trChildNode[0].textContent);
		$("#POSITION_LEVEL").val(trChildNode[1].textContent);
		$("#POSITION_NAME").val(trChildNode[2].textContent);
		updatePOSITION_NAME = trChildNode[2].textContent;
	}
}
...
function updateEPositionWithMenu(){
	...
	for (let checkedNode of checkedNodes) {
		param=param+"MENU_ID:"+checkedNode.MENU_ID+",";
	}
	$.ajax({
